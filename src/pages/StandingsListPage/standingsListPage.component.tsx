...
export const StandingsListPage = memo(function StandingsListPage() {
  ...
  const { content, pagination, loading } = useGetDriverStandingsQuery({ limit: SEASONS_PER_PAGE, offset }, {
  ...
  const itemCount: number = useMemo(() => (
    pagination ? (Number(pagination.total) - INIT_OFFSET) : 0
  ), [pagination]);
